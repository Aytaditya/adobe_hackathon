...
export default function PDFManager() {
  ...
  // Mock data for existing PDFs (kept as is, as backend response is for uploaded files)
  const existingPdfs = [
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      // Clear any previous backend response when new files are selected
  ...
  const removeFile = (index: number) => {
    ...
    // If all files are removed, clear backend response
  }
  ...
  const handleUploadAndProcess = async () => {
    ...
    try {
      ...
      // Store analysis data in localStorage for the chat page
  ...
  // Navigate to chat page with processed files
  const navigateToChatWithProcessedFiles = () => {
                  ...
                  {files && files.length > 0 && (
                    <div className="mt-8 text-left">
                      ...
                      <div className="mt-4 flex justify-between items-center">
                        ...
                        <button
                          onClick={handleUploadAndProcess} // Changed to new handler
                        ...
                        >
                          {isProcessing
                            ? "Processing..."
                            : `Chat with ${files.length} PDF${files.length > 1 ? "s" : ""}`}
                        </button>
                      </div>
                    </div>
                  )}
